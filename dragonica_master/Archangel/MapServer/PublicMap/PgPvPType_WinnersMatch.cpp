...
bool PgPvPType_WinnersMatch::SortEntry(SPvPGameUser const &lhs, SPvPGameUser const &rhs)
{
...
PgPvPType_WinnersMatch::PgPvPType_WinnersMatch()
{
...
HRESULT PgPvPType_WinnersMatch::Init( BM::Stream &kPacket, PgWarGround* pkGnd )
{
...
void PgPvPType_WinnersMatch::InitEntry()
{
	...
	kContTeamRed.sort(PgPvPType_WinnersMatch::SortEntry);
	kContTeamBlue.sort(PgPvPType_WinnersMatch::SortEntry);

	m_kContUserEntry.insert( std::make_pair(TEAM_RED, kContTeamRed) );
	m_kContUserEntry.insert( std::make_pair(TEAM_BLUE, kContTeamBlue) );
}

void PgPvPType_WinnersMatch::InitTeamResult()
{
...
bool PgPvPType_WinnersMatch::IsStart( DWORD const dwNowTime )
{
	return m_kContReady.empty();
}

HRESULT PgPvPType_WinnersMatch::Join( PgPlayer *pkPlayer, bool const bInturde )
{
	if ( pkPlayer )
	{
...
BM::GUID PgPvPType_WinnersMatch::GetTopEntryUser( int const iTeam )
{
...
SPvPGameUser PgPvPType_WinnersMatch::PopTopEntryUser( int const iTeam )
{
	SPvPGameUser kUser;
...
bool PgPvPType_WinnersMatch::Start( PgPlayer *pkPlayer, PgWarGround * pkGnd )
{
...
HRESULT PgPvPType_WinnersMatch::IsObModePlayer( PgPlayer *pkPlayer, BM::GUID& rkOutTargetGuid )
...
HRESULT PgPvPType_WinnersMatch::SetEvent_Kill( CUnit *pkUnit, CUnit *pkKiller, PgWarGround *pkGnd )
{
	if ( pkUnit )
	{
		...
		if ( die_itr != m_kContPlay.end() )
		{
			...
			EntryChange( kElement.kTeamSlot.GetTeam(), kKey );
...
HRESULT PgPvPType_WinnersMatch::Update( DWORD const dwCurTime )
{
	if ( m_kMode == PVP_MODE_TEAM )
	{
...
bool PgPvPType_WinnersMatch::EntryChange(int const iTeam, BM::GUID const& kCharGuid)
{
...
void PgPvPType_WinnersMatch::SetWinTeam(int const iWinTeam)
{
	if ( m_kMode == PVP_MODE_TEAM )
	{
...
void PgPvPType_WinnersMatch::SetLoseTeam(int const iLoseTeam)
{
	if ( m_kMode == PVP_MODE_TEAM )
	{
...
bool PgPvPType_WinnersMatch::CalcRoundWinTeam( int &iOutRoundWinTeam )
{
	bool bEndGame = false;
	if ( m_kMode == PVP_MODE_TEAM )
	{
...
bool PgPvPType_WinnersMatch::InitUnitInfo( UNIT_PTR_ARRAY const &kUnitArray, PgWarGround *pkGnd )
{
	m_kContWait.clear();
...
bool PgPvPType_WinnersMatch::IsAbusing(void)const
{
...
void PgPvPType_WinnersMatch::StartGame( BM::Stream &kPacket, bool const bGame )
{
	PgPvPType_DeathMatch::StartGame( kPacket, bGame );
}


bool PgPvPType_WinnersMatch::End( bool const bAutoChange, PgWarGround *pkGnd )
{
	// 이긴팀 계산을 해야 한다.
...
EResultEndType PgPvPType_WinnersMatch::ResultEnd()
{
	if ( IsEndGame() )
	{
