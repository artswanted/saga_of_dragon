...
bool CALLBACK OnF1( unsigned short const &rkInputKey)
{
	...
	kOrderData.Type = ORDER_CASHSHOP_TABLE_RELOAD;
	kOrderData.kReqGuid = BM::GUID::Create();
...
bool CALLBACK OnF2( unsigned short const &rkInputKey)
{
	SGmOrder kOrderData;
	BM::Stream kPacket(PT_GM_A_REQ_GMORDER);
	kOrderData.Type = ORDER_EVENT_ITEM_REWARD_RELOAD;
	kOrderData.kReqGuid = BM::GUID::Create();
	kOrderData.WriteToPacket(kPacket);
	SendToImmigration(kPacket);
	return true;
...
bool CALLBACK OnF3( unsigned short const &rkInputKey)
{
	SGmOrder kOrderData;
	BM::Stream kPacket(PT_GM_A_REQ_GMORDER);
	kOrderData.Type = ORDER_GAMBLEMACHINE_RELOAD;
	kOrderData.kReqGuid = BM::GUID::Create();
	kOrderData.WriteToPacket(kPacket);
	SendToImmigration(kPacket);
	return true;
