...
using namespace HSEL;

static CHSEL::KEY_TYPE GetNextKey(CHSEL::KEY_TYPE const& A, CHSEL::GAB_TYPE const& B, CHSEL::GAB_TYPE const& C)	
{
	return ( (A)*(B) ) + (C);
}
...
bool CHSEL::EncryptInitInfo(INIT_INFO &rInitInfo)const
{
...
bool CHSEL::DecryptInitInfo(INIT_INFO &rInitInfo)const
{
	DWORD const dwBlockCount	= (sizeof(INIT_INFO)-sizeof(INIT_INFO_KEY_TYPE)-sizeof(INIT_INFO_CRC_TYPE)) / sizeof(INIT_INFO_KEY_TYPE) ;
	DWORD dwRemain				= (sizeof(INIT_INFO)-sizeof(INIT_INFO_KEY_TYPE)-sizeof(INIT_INFO_CRC_TYPE)) % sizeof(INIT_INFO_KEY_TYPE) ;
...
CHSEL::eRESULT_CODE CHSEL::Init(DWORD const& dwInitFlag)
{
	...
	case ENCRYPT_FAST:
		{
			m_pFuncEn1 = &CHSEL::REN;
			m_pFuncEn2 = NULL;
			m_pFuncEn3 = NULL;

			m_pFuncDe1 = &CHSEL::RDE;
			m_pFuncDe2 = NULL;
			m_pFuncDe3 = NULL;
		}break;
	case ENCRYPT_SECURE:
		{
			m_pFuncEn1 = &CHSEL::REN;
			m_pFuncEn2 = &CHSEL::DLEN;
			m_pFuncEn3 = &CHSEL::DREN;

			m_pFuncDe1 = &CHSEL::RDE;
			m_pFuncDe2 = &CHSEL::DLDE;
			m_pFuncDe3 = &CHSEL::DRDE;
		}break;
	default:
		{
	...
	case SWAP_ON:
		{
			m_pFuncEnSwap = &CHSEL::SwapEn;
			m_pFuncDeSwap = &CHSEL::SwapDe;
		}break;
	case SWAP_OFF:
		{
			m_pFuncEnSwap = NULL;
...
CHSEL::eRESULT_CODE CHSEL::InitManual(INIT_INFO const& rInitInfo)
{
	BM::CAutoMutex kLock(m_kMutex);
...
bool CHSEL::GetInitInfo(INIT_INFO &rInitInfo)const//이걸 인코드 할때 써야함.
{
	BM::CAutoMutex kLock(m_kMutex);
...
bool CHSEL::Encrypt(char *pSrc, DWORD const dwSrcSize, DWORD &dwOutCRC)
{
	BM::CAutoMutex kLock(m_kMutex);
...
bool CHSEL::Encrypt(char const *pSrc, char *pDest, DWORD const dwSrcSize, DWORD &dwOutCRC)
{
	BM::CAutoMutex kLock(m_kMutex);
...
bool CHSEL::Decrypt(char *pSrc, DWORD const dwSrcSize, DWORD &dwOutCRC)
{
	BM::CAutoMutex kLock(m_kMutex);
...
bool CHSEL::Decrypt(char const *pSrc, char *pDest, DWORD const dwSrcSize, DWORD &dwOutCRC)
{
	BM::CAutoMutex kLock(m_kMutex);
	::memcpy(pDest, pSrc, dwSrcSize);
...
CHSEL::GAB_TYPE CHSEL::MakeGab()
{
	return 	static_cast<CHSEL::GAB_TYPE>(BM::Rand_Range(UCHAR_MAX, 1));
}
...
void CALLBACK CHSEL::REN(char *pSrc, DWORD const dwSrcSize)
...
void CALLBACK CHSEL::DLEN(char *pSrc, DWORD const dwSrcSize)
{//DES 알고리즘
	if(1 > dwSrcSize){return;}
...
void CALLBACK CHSEL::DREN(char *pSrc, DWORD const dwSrcSize)
{//DES 알고리즘
	if(1 > dwSrcSize){return;}
...
void CALLBACK CHSEL::RDE(char *pSrc, DWORD const dwSrcSize)
...
void CALLBACK CHSEL::DLDE(char *pSrc, DWORD const dwSrcSize)
{
	//마지막 블럭에서 키를 찾아낸다.
...
void CALLBACK CHSEL::DRDE(char *pSrc, DWORD const dwSrcSize)
{
	if(1 > dwSrcSize){return;}
...
void CALLBACK CHSEL::SwapEn(char *pSrc, DWORD const dwSrcSize)
{	//블럭끼리 혹은 바이트만.
...
void CALLBACK CHSEL::SwapDe(char *pSrc, DWORD const dwSrcSize)
{	//블럭끼리 혹은 바이트만.
	//키 값은 현재 MAX_RD_KEY_ARRAY + LDES + RDES * 2(Gab),  확보중
	//륀다엘 키와 Gab을 이용해서 Swap을 진행
	DWORD const dwBlockCount	= ( dwSrcSize / sizeof(KEY_TYPE) ) ;//키의 크기로 나눔.
...
CHSEL::KEY_TYPE CALLBACK CHSEL::GetDataCRC(char *pSrc, DWORD const dwSrcSize)
{
	if(1 > dwSrcSize){return 0;}
	...
	{
		char* pRef = pSrc + dwRemain - 1;//헤드 짜투리. 배열이므로 -1을 해야 한다.
		char* pCRC = reinterpret_cast<char*>(&CRC) + sizeof(KEY_TYPE) - 1;
		while(true)
		{
			*pCRC  ^= *pRef;
...
WORD CHSEL::CRCToWORD(DWORD const& kInCrc)
{
	return (HIWORD(kInCrc) ^ LOWORD(kInCrc));
}

BYTE CHSEL::CRCToByte(DWORD const& kInCrc)
{
