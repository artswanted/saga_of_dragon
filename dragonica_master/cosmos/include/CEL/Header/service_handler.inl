...
Service_Handler< T_SESSION >::Service_Handler(INIT_CORE_DESC const& kInitInfo)
...
template< typename T_SESSION >
Service_Handler< T_SESSION >::~Service_Handler(void)
{
...
template< typename T_SESSION >
void Service_Handler< T_SESSION >::Clear()
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
HRESULT Service_Handler< T_SESSION >::NewBlk(MSG_BLK* &pkBlk, size_t const blk_size)const
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
void Service_Handler< T_SESSION >::ClearBlk(MSG_BLK* &pkBlk)const
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
	//std::cout<< __FUNCTION__<<":"<<__LINE__ <<":"<< this <<std::endl;
	while(pkBlk)
...
template< typename T_SESSION >
void Service_Handler< T_SESSION >::AdjustBlk(MSG_BLK* &pkBlk)const
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
	//std::cout<< __FUNCTION__<<":"<<__LINE__ <<":"<< this <<std::endl;
	while(pkBlk)
...
template< typename T_SESSION >
void Service_Handler< T_SESSION >::open(ACE_HANDLE new_handle, MSG_BLK &message_block)
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
__OPEN_FAILED:
	{
		CEL_LOG( BM::LOG_LV6, __FL__ << L"Call TryClose " << m_kAddr.ToString() << L"Pointer: " << this << L" Cause: " << eRet);
		TryClose(E_ONCLOSE_BY_OPEN);
	}
}

template< typename T_SESSION >
bool Service_Handler< T_SESSION >::PostWrite( MSG_BLK *pMsgBlk )//이걸 쓰는곳에서 WriteLock을 걸고 불러라.
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
	E_RESULT_CODE eRet = E_R_NONE;
...
template< typename T_SESSION >
void Service_Handler< T_SESSION >::handle_write_stream(const ACE_Asynch_Write_Stream::Result &result)
...
template< typename T_SESSION >
bool Service_Handler< T_SESSION >::PostRead()//! ! Recv
...
template< typename T_SESSION >
void Service_Handler< T_SESSION >::handle_read_stream(const ACE_Asynch_Read_Stream::Result &result)
...
{//! ! Guard 문제로.
	...
	m_kRecvPacketBuf.Clear();
	HEADER_TYPE kHeader;
	while( PopRecved(m_kRecvPacketBuf, kHeader) )
	{
		//DWORD const dwStart = BM::GetTime32();
		
		if( m_kInitInfo.OnRecv )//! 
		{
			if( m_kRecvPacketBuf.Size() )//내용이 없는 경우가 있다 (EX> 타임아웃 체크)
			{
				if(S_OK == VDecodeRecved(m_kRecvPacketBuf, reinterpret_cast<void*>(&kHeader)))
				{
					m_kInitInfo.OnRecv(this, &m_kRecvPacketBuf);
				}
				else
				{//패킷 풀기 실패.
					iRet = 4;
...
__ON_CLOSE://! ! PostRead를 호출 했으나 실패한 경우.
	{//read 는 셧다운이 필요 없다.
		...
		CEL_LOG( BM::LOG_LV6, __FL__ << L"SessionClose " << m_kAddr.ToString() << L"Pointer: " << this << L" iRet: " << iRet);
...
template< typename T_SESSION >
bool Service_Handler< T_SESSION >::PopRecved( PACKET_TYPE &rOutPacket, HEADER_TYPE &kHeader)
{//! ! 패킷 하나를 빼준다. 메모리를 복사해주고 블럭은 돌려줘버린다
	...
	if( PopFromHeader( &header, sizeof(HEADER_TYPE), false ) )//! ! 헤더 길이는 있네.
	{
		...
		if( header.BodySize() + sizeof(HEADER_TYPE) <= total_blk_length)
		{//100kb 정도로 줄여놨으니 죽을 일은 없겠다.
			rOutPacket.Clear();
			rOutPacket.Resize( header.BodySize() );//! ! 메모리 예약

			PopFromHeader( &header, sizeof(HEADER_TYPE), true );
			if( header.BodySize() )//길이가 있다면
			{
				PopFromHeader( &rOutPacket.Data().at(0), header.BodySize(), true );
...
template< typename T_SESSION >
bool Service_Handler< T_SESSION >::PopFromHeader( void* pDst, size_t const size, bool const bPtrMove )
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
void Service_Handler< T_SESSION >::TryClose(const E_RESULT_CODE eCause)//! ! 접속이 끊어지면 호출 됨. 사용중인 메세지 블럭도 돌려주는 처리.
...
__DELETE_THIS:
	{
		...
		CEL_LOG( BM::LOG_LV6, __FL__ << L"SessionClose " << m_kAddr.ToString() << L"Pointer: " << this << L" Cause: " << eCause);
...
template< typename T_SESSION >
void Service_Handler< T_SESSION >::ShutDown(int const iCallType)
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
void Service_Handler< T_SESSION >::addresses(ACE_INET_Addr const& rRemoteAddr, ACE_INET_Addr const& rLocalAddr)
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
ACE_INET_Addr const& Service_Handler< T_SESSION >::RemoteAddr() const {return m_kRemoteAddr;}


template< typename T_SESSION >
void Service_Handler< T_SESSION >::act (void const *pAct)
{//! ! Onconnect 시에 호출
	IG_LOG LockCheck( m_kDebugLog, kCheck );
	if( pAct )
	{
...
template< typename T_SESSION >
bool Service_Handler< T_SESSION >::VSend( PACKET_TYPE const& rOrgPacket, bool const bIsEncryption)
{
	...
	if(S_OK == NewBlk(pkMsgBlk, sizeof(HEADER_TYPE) + imp_data_size))
	{
		...
		HEADER_TYPE *pHeader = reinterpret_cast< HEADER_TYPE* >(pkMsgBlk->wr_ptr());//헤더 셋팅
...
template< typename T_SESSION >
bool Service_Handler< T_SESSION >::VTerminate()
{
	if(m_bIsAlive)
	{
		ShutDown(0);
...
template< typename T_SESSION >
HRESULT Service_Handler< T_SESSION >::Statistics(SSessionStatistics &rkOutResult) const
{
...
template< typename T_SESSION >
bool Service_Handler< T_SESSION >::IsCorrectBandwidth(__int64 const iRecvBytes)
{
	...
	if(rettime > ACE_Time_Value(10) )//1초 넘을때 체크.
	{
		...
		{
			CEL_LOG(BM::LOG_LV6, "[bandwidth] too fast: " << bandwidth << " Pointer: " << this);
			return false;
		}
