...
Service_Handler_Gravity< T_SESSION >::Service_Handler_Gravity(INIT_CORE_DESC const& kInitInfo)
...
template< typename T_SESSION >
Service_Handler_Gravity< T_SESSION >::~Service_Handler_Gravity(void)
{
...
template< typename T_SESSION >
void Service_Handler_Gravity< T_SESSION >::Clear()
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
HRESULT Service_Handler_Gravity< T_SESSION >::NewBlk(MSG_BLK* &pkBlk, size_t const blk_size)const
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
void Service_Handler_Gravity< T_SESSION >::ClearBlk(MSG_BLK* &pkBlk)const
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
	//std::cout<< __FUNCTION__<<":"<<__LINE__ <<":"<< this <<std::endl;
	while(pkBlk)
...
template< typename T_SESSION >
void Service_Handler_Gravity< T_SESSION >::AdjustBlk(MSG_BLK* &pkBlk)const
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
	//std::cout<< __FUNCTION__<<":"<<__LINE__ <<":"<< this <<std::endl;
	while(pkBlk)
...
template< typename T_SESSION >
void Service_Handler_Gravity< T_SESSION >::open(ACE_HANDLE new_handle, MSG_BLK &message_block)
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
bool Service_Handler_Gravity< T_SESSION >::PostWrite( MSG_BLK *pMsgBlk )//이걸 쓰는곳에서 WriteLock을 걸고 불러라.
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
	E_RESULT_CODE eRet = E_R_NONE;
...
template< typename T_SESSION >
void Service_Handler_Gravity< T_SESSION >::handle_write_stream(const ACE_Asynch_Write_Stream::Result &result)
...
template< typename T_SESSION >
bool Service_Handler_Gravity< T_SESSION >::PostRead()//! ! Recv
...
template< typename T_SESSION >
void Service_Handler_Gravity< T_SESSION >::handle_read_stream(const ACE_Asynch_Read_Stream::Result &result)
...
template< typename T_SESSION >
bool Service_Handler_Gravity< T_SESSION >::PopRecved( PACKET_TYPE &rOutPacket, HEADER_TYPE &kHeader)
	...
	if( PopFromHeader( &header, sizeof(HEADER_TYPE), false ) )//! ! 헤더 길이는 있네.
	{
		...
		{	
			...
			if( iBodySize > 0 )//길이가 있다면
			{
				...
				PACKET_TYPE kTempPacket;
				kTempPacket.Push(header.acCommand);
				kTempPacket.Push(rOutPacket);
				
				rOutPacket = kTempPacket;
				rOutPacket.PosAdjust();
			}
...
template< typename T_SESSION >
bool Service_Handler_Gravity< T_SESSION >::PopFromHeader( void* pDst, size_t const size, bool const bPtrMove )
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
void Service_Handler_Gravity< T_SESSION >::TryClose(const E_RESULT_CODE eCause)//! ! 접속이 끊어지면 호출 됨. 사용중인 메세지 블럭도 돌려주는 처리.
...
template< typename T_SESSION >
void Service_Handler_Gravity< T_SESSION >::ShutDown(int const iCallType)
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
void Service_Handler_Gravity< T_SESSION >::addresses(ACE_INET_Addr const& rRemoteAddr, ACE_INET_Addr const& rLocalAddr)
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
ACE_INET_Addr const& Service_Handler_Gravity< T_SESSION >::RemoteAddr() const {return m_kRemoteAddr;}


template< typename T_SESSION >
void Service_Handler_Gravity< T_SESSION >::act (void const *pAct)
{//! ! Onconnect 시에 호출
	IG_LOG LockCheck( m_kDebugLog, kCheck );
	if( pAct )
	{
...
template< typename T_SESSION >
bool Service_Handler_Gravity< T_SESSION >::VSend( PACKET_TYPE const& rOrgPacket, bool const bIsEncryption)
{
	...
	kHeader.BodySize(kHeader.BodySize()-4); // 패킷타입 4바이트 빼야 함.
	kPacket.PopMemory(kHeader.acCommand, sizeof(kHeader.acCommand));

	size_t const imp_data_size = kPacket.Data().size()-4; // 맨처음 4바이트는 패킷타입이라 빼준다. imp_data_size는 순수 Body size
	...
	if(S_OK == NewBlk(pkMsgBlk, sizeof(HEADER_TYPE) + imp_data_size))
	{
		...
		*pHeader = kHeader;
		...
		if(imp_data_size)
		{
			...
			::memcpy(pBody, &kPacket.Data().at(4), imp_data_size);
...
template< typename T_SESSION >
bool Service_Handler_Gravity< T_SESSION >::VTerminate()
{
	if(m_bIsAlive)
	{
		ShutDown(0);
...
template< typename T_SESSION >
HRESULT Service_Handler_Gravity< T_SESSION >::Statistics(SSessionStatistics &rkOutResult) const
{
...
template< typename T_SESSION >
bool Service_Handler_Gravity< T_SESSION >::IsCorrectBandwidth(__int64 const iRecvBytes)
{
