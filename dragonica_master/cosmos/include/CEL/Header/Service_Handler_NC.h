...
namespace CEL
{
	...
	class Service_Handler_NC//! 가능한한 최소한의 네트웍 기능을 가진다.
...
//		typedef typename ACE_Recursive_Thread_Mutex MUTEX;
	...
	public:
		...
		typedef typename Service_Handler_NC< T_SESSION >					THIS_TYPE;
	...
	public:
		Service_Handler_NC(INIT_CORE_DESC const& kInitInfo = INIT_CORE_DESC());
		virtual ~Service_Handler_NC(void);
	...
	protected:
		...
		static short const HEADER_ADDED_VALUE = 2;	// NC 에서는 Header_Size값이 Header_Size 자신의 크기까지 포함해야 한다.
	};

#include "Service_Handler_NC.inl"
}; 