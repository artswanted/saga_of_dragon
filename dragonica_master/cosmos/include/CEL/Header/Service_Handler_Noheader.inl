...
Service_Handler_NoHeader< T_SESSION >::Service_Handler_NoHeader(INIT_CORE_DESC const& kInitInfo)
...
template< typename T_SESSION >
Service_Handler_NoHeader< T_SESSION >::~Service_Handler_NoHeader(void)
{
...
template< typename T_SESSION >
void Service_Handler_NoHeader< T_SESSION >::Clear()
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
HRESULT Service_Handler_NoHeader< T_SESSION >::NewBlk(MSG_BLK* &pkBlk, size_t const blk_size)const
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
void Service_Handler_NoHeader< T_SESSION >::ClearBlk(MSG_BLK* &pkBlk)const
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
	//std::cout<< __FUNCTION__<<":"<<__LINE__ <<":"<< this <<std::endl;
	while(pkBlk)
...
template< typename T_SESSION >
void Service_Handler_NoHeader< T_SESSION >::AdjustBlk(MSG_BLK* &pkBlk)const
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
	//std::cout<< __FUNCTION__<<":"<<__LINE__ <<":"<< this <<std::endl;
	while(pkBlk)
...
template< typename T_SESSION >
void Service_Handler_NoHeader< T_SESSION >::open(ACE_HANDLE new_handle, MSG_BLK &message_block)
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
bool Service_Handler_NoHeader< T_SESSION >::PostWrite( MSG_BLK *pMsgBlk )//이걸 쓰는곳에서 WriteLock을 걸고 불러라.
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
	E_RESULT_CODE eRet = E_R_NONE;
...
template< typename T_SESSION >
void Service_Handler_NoHeader< T_SESSION >::handle_write_stream(const ACE_Asynch_Write_Stream::Result &result)
...
template< typename T_SESSION >
bool Service_Handler_NoHeader< T_SESSION >::PostRead()//! ! Recv
...
template< typename T_SESSION >
void Service_Handler_NoHeader< T_SESSION >::handle_read_stream(const ACE_Asynch_Read_Stream::Result &result)
...
template< typename T_SESSION >
bool Service_Handler_NoHeader< T_SESSION >::PopRecved( PACKET_TYPE &rOutPacket, HEADER_TYPE &kHeader)
{//! ! 패킷 하나를 빼준다. 메모리를 복사해주고 블럭은 돌려줘버린다
	IG_LOG LockCheck( m_kDebugLog, kCheck );

	size_t const total_blk_length = m_pRecvHeadBlk->total_length();

	rOutPacket.Clear();
	if (total_blk_length > 0)
	{
		rOutPacket.Resize( total_blk_length );//! ! 메모리 예약
		PopFromHeader( &rOutPacket.Data().at(0), total_blk_length, true );
...
template< typename T_SESSION >
bool Service_Handler_NoHeader< T_SESSION >::PopFromHeader( void* pDst, size_t const size, bool const bPtrMove )
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
void Service_Handler_NoHeader< T_SESSION >::TryClose(const E_RESULT_CODE eCause)//! ! 접속이 끊어지면 호출 됨. 사용중인 메세지 블럭도 돌려주는 처리.
...
template< typename T_SESSION >
void Service_Handler_NoHeader< T_SESSION >::ShutDown(int const iCallType)
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
void Service_Handler_NoHeader< T_SESSION >::addresses(ACE_INET_Addr const& rRemoteAddr, ACE_INET_Addr const& rLocalAddr)
{
	IG_LOG LockCheck( m_kDebugLog, kCheck );
...
template< typename T_SESSION >
ACE_INET_Addr const& Service_Handler_NoHeader< T_SESSION >::RemoteAddr() const {return m_kRemoteAddr;}


template< typename T_SESSION >
void Service_Handler_NoHeader< T_SESSION >::act (void const *pAct)
{//! ! Onconnect 시에 호출
	IG_LOG LockCheck( m_kDebugLog, kCheck );
	if( pAct )
	{
...
template< typename T_SESSION >
bool Service_Handler_NoHeader< T_SESSION >::VSend( PACKET_TYPE const& rOrgPacket, bool const bIsEncryption)
{
	...
	if(MAX_BODY_SIZE < imp_data_size)
	{//비트팩이라 사이즈가 안될 수도 있음
	...
	if(S_OK == NewBlk(pkMsgBlk, imp_data_size))
	{
...
template< typename T_SESSION >
bool Service_Handler_NoHeader< T_SESSION >::VTerminate()
{
	if(m_bIsAlive)
	{
		ShutDown(0);
...
template< typename T_SESSION >
HRESULT Service_Handler_NoHeader< T_SESSION >::Statistics(SSessionStatistics &rkOutResult) const
{
...
template< typename T_SESSION >
bool Service_Handler_NoHeader< T_SESSION >::IsCorrectBandwidth(__int64 const iRecvBytes)
{
