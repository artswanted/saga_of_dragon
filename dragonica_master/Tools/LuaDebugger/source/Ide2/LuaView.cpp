...
BEGIN_MESSAGE_MAP(CLuaView, CView)
	...
	ON_COMMAND(ID_EDIT_REMOVEALLBREAKPOINT, OnEditRemoveAllBP)
	ON_UPDATE_COMMAND_UI(ID_EDIT_REMOVEALLBREAKPOINT, OnUpdateRemoveAllBP)
...
// ide2.rc 에 해당 단축키를 지정해준다.
void CLuaView::OnEditFind() 
{
	CPgFindDlg *m_pFindDlg;
	m_pFindDlg = new CPgFindDlg;
	m_pFindDlg->Create(IDD_DLG_FIND, this);
	m_pFindDlg->SetLuaEditorPtr(&m_editor);
	m_pFindDlg->ShowWindow(SW_SHOW);
	//m_pFindDlg->ShowWindowPos(HWND_TOPMOST);
	
	//int asdf = 123;
	//m_editor.Find();
}

void CLuaView::OnUpdateEditFind(CCmdUI* pCmdUI) 
{
	int asdf = 123;
	//pCmdUI->Enable(m_editor.CanRedo());
}

void CLuaView::OnEditRemoveAllBP()
{
	if (IDOK == ::MessageBox(0, "브레이크 포인트가 모두 지워집니다.", "브레이크 포인트 모두 지우기", MB_OKCANCEL))
	{
		m_editor.ClearAllBreakpoints();
	}
}

void CLuaView::OnUpdateRemoveAllBP(CCmdUI* pCmdUI)
{
}
...
void CLuaView::ToggleBreakPoint(int nLine)
{
	...
	// 브레이크 포인트 변경시
	// 데이터 다시 전송 해주기.
	int a = pFrame->GetMode();
	if ( pFrame->GetMode() == CMainFrame::modeDebug &&
		pFrame->GetMode() != CMainFrame::modeDebugBreak )
	{
		// Clear
		PgLuaDebugPacket kPacket;
		kPacket.ePacketType = PgLuaDebugPacket::PacketType_Debug_LineClear;
		g_pkPgServer->Send(sizeof(PgLuaDebugPacket), ((char *)&kPacket));

		// Refresh
		g_pkPgServer->SendBreakData(PgDebugInfo::PacketType_Debug_LineOnlyRefresh);
	}
}
