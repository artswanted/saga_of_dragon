...
LRESULT CMainFrame::DebugMessage(UINT nMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case DMSG_GOTO_FILELINE:
		...
		break;
	case DMSG_GOTO_FILELINE_FORDEBUG:
		GotoFileLineForDebug((const char*)wParam, (int)lParam);
		break;
	...
	case DMSG_REDRAW_WATCHES:
		m_wndWatches.Redraw();
		break;
	case DMSG_RESEND_WATCHES:
		m_wndWatches.ReSendWatchData();
		break;
	}
...
void CMainFrame::GotoFileLineForDebug(const char* szFile, int nLine)
{
	char szCurDir[512];
	char szFullPath[512];
	...
	::SetCurrentDirectory(pProject->GetExecuteDir());

 	::GetFullPathName(szFile,512,szFullPath,NULL);
	CProjectFile* pPF = GetProject()->GetProjectFile(szFullPath);
	if ( pPF )
		theApp.OpenProjectFilesView(pPF, nLine);
...
void CMainFrame::UpdateFrameTitleForDocument(LPCTSTR lpszDocName)
{
	...
	else
	{
		...
		else if ( m_nAppMode==modeBuild )
			lstrcat(szText, _T(" [end] "));
