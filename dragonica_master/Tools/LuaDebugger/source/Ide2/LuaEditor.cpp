...
bool CLuaEditor::Find(char *szToFindText, BOOL bOnlyCorrect)
{
	TextToFind *tf = new TextToFind;
	tf->lpstrText = szToFindText;
	tf->chrg.cpMin = Sci(SCI_GETCURRENTPOS);
	tf->chrg.cpMax = Sci(SCI_GETLENGTH);

	int wParam = SCFIND_REGEXP;
	if (bOnlyCorrect == TRUE)
		wParam = SCFIND_MATCHCASE;

	int pos = Sci(SCI_FINDTEXT, wParam, (int)tf);
	if (pos != -1)
	{
		Sci(SCI_GOTOPOS, pos);
		Sci(SCI_SETSELECTIONEND, pos + strlen(szToFindText));
