...
function Effect_Begin106500801(unit, iEffectNo, actarg)
	local kEffectMgr = unit:GetEffectMgr()
	local kEffect = kEffectMgr:GetEffect(iEffectNo, false)
	if kEffect:IsNil() == true then
		InfoLog(5, "Effect_Begin106500801...Cannot GetEffect(" .. iEffectNo ..")");
		return 0
	end
	-- 공격속도 증가
...
function Effect_End106500801(unit, iEffectNo, actarg)
	local kEffectMgr = unit:GetEffectMgr()
	local kEffect = kEffectMgr:GetEffect(iEffectNo, false)
	if kEffect:IsNil() == true then
		InfoLog(5, "Effect_End106500801...Cannot GetEffect(" .. iEffectNo ..")");
		return 0
	end
	-- 공격속도 감소
	local iAdd = kEffect:GetAbil(AT_R_ATTACK_SPEED)
...
function Effect_Tick106500801(unit, elapsedtime, effect, actarg)
	InfoLog(6, "Don't call me Effect_Tick106500801 Effect=" .. effect:GetEffectNo())
	return ECT_DOTICK
end
