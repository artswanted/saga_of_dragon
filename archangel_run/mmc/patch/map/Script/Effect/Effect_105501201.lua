...
-- Paladin (디펜시브 오라 : a_Offecsive Aura) : Level 1-5
function Effect_Begin105501201(unit, iEffectNo, actarg)
--	local effect = unit:GetEffect(iEffectNo);
--	local kEffectTargetList = effect:GetTargetList();

	--InfoLog(5, "Effect_Begin105501201 : iEffectNo : "..iEffectNo)

--	kEffectTargetList:Add(unit:GetGuid());

	-- 실제 효과 이펙트	
--	local iNewEffectNo = effect:GetAbil(AT_EFFECTNUM1);
	--InfoLog(5, "Effect_Begin105501201 : iNewEffectNo : "..iNewEffectNo)
--	unit:AddEffect(iNewEffectNo, 0, actarg, unit)
	
	return 1
end
                                                                                                                          
function Effect_End105501201(unit, iEffectNo, actarg)
	local effect = unit:GetEffect(iEffectNo);
...
function Effect_Tick105501201(unit, elapsedtime, effect, actarg)
	...
	--InfoLog(5, "Effect_Tick105501201 : iNewEffectNo : "..iNewEffectNo)
	--NeedMP값이 -로 세팅되어있다.
	if -iNeed > iMP then
		--InfoLog(5, "Effect_Tick105501201 : NeedMP : "..iNeed.. "  MP : "..iMP)
	...
	if kGround:IsNil() then
		InfoLog(5, "Effect_Tick105501201 Ground is NILL")
		return ECT_MUSTDELETE
	end
	...
	if unit:GetPartyGuid():IsNil() == false then
		...
		--InfoLog(5, "Effect_Tick105501201 : iRange "..iRange)
		while kMemberGuid:IsNil() == false do
			--InfoLog(5, "Effect_Tick105501201 : Party: index "..iIndex)
			-- 타겟 리스트에 없으면
			if false == kEffectTargetList:IsGuid(kMemberGuid) then
				--InfoLog(5, "Effect_Tick105501201 : kEffectTargetList:IsGuid() == false")
				...
				if kMember:IsNil() == false then
					...
					if IsInRange(kMyPos, kMember:GetPos(), iRange, 30) == true then
						--InfoLog(5, "Effect_Tick105501201 : Not InTargetList : InRange ")
			...
			-- 타겟 리스트에 있으면
				...
				if kMember:IsNil() == false then
					...
					if IsInRange(kMyPos, kMember:GetPos(), iRange, 30) == false then
						--InfoLog(5, "Effect_Tick105501201 : InTargetList : Not InRange ")
						kEffectTargetList:Delete(kMemberGuid);
						kMember:DeleteEffect(iNewEffectNo);
					-- Effect 사거리안에 있으면
					else
						--InfoLog(5, "Effect_Tick105501201 : InTargetList : InRange ")
	...
	--InfoLog(9, "Effect_Tick105501201 NeedMP : " .. iNeed.."   C_MP:"..iMP)
...
function Effect_Begin1055012001(unit, iEffectNo, actarg)
	local kEffectMgr = unit:GetEffectMgr()
	local kEffect = kEffectMgr:GetEffect(iEffectNo, false)
	if kEffect:IsNil() == true then
		InfoLog(5, "Effect_Begin1055012001...Cannot GetEffect(" .. iEffectNo ..")");
		return 0
	end
...
function Effect_End1055012001(unit, iEffectNo, actarg)
	local kEffectMgr = unit:GetEffectMgr()
	local kEffect = kEffectMgr:GetEffect(iEffectNo, false)
	if kEffect:IsNil() == true then
		InfoLog(5, "Effect_End1055012001...Cannot GetEffect(" .. iEffectNo ..")");
		return 0
	end
...
function Effect_Tick1055012001(unit, elapsedtime, effect, actarg)
	InfoLog(6, "Don't call me Effect_Tick1055012001 Effect=" .. effect:GetEffectNo())
	return ECT_DOTICK
end
