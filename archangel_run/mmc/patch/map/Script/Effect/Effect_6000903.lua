...
--[===[	
	...
	local iIndex = 0
	local iNbLine = lavalon:GetAbil(AT_SKILL_CUSTOM_DATA_01)
	
	while iIndex < iNbLine do
		InfoLog(9, "Skill_Fire900000031 2.1")
		local fMetaForStart = lavalon:GetAbil(AT_SKILL_CUSTOM_DATA_01 + iIndex * 2 + 1) / 1000
		local fMetaForEnd = lavalon:GetAbil(AT_SKILL_CUSTOM_DATA_01 + iIndex * 2 + 2) / 1000
...
function Effect_End6000903(lavalon, iEffectNo, actarg)
	return 1
	--return Effect_End6000900(lavalon, iEffectNo, actarg)
end

function Effect_Tick6000903(lavalon, elapsedtime, effect, actarg)
