...
function Effect_Begin1090004001(unit, iEffectNo, actarg)
	local kEffectMgr = unit:GetEffectMgr()
	local kEffect = kEffectMgr:GetEffect(iEffectNo, false)
	if kEffect:IsNil() == true then
		InfoLog(5, "Effect_Begin1090004001...Cannot GetEffect(" .. iEffectNo ..")");
		return 0
	end
	local iAdd = kEffect:GetAbil(AT_R_HITRATE)
	InfoLog(9, "Effect_Begin1090004001 Before:AT_C_HITRATE:" .. unit:GetAbil(AT_C_HITRATE))
	kEffectMgr:AddAbil(AT_R_HITRATE, iAdd)
	unit:NftChangedAbil(AT_R_HITRATE, E_SENDTYPE_NONE)
	InfoLog(9, "Effect_Begin1090004001 After:AT_C_HITRATE:" .. unit:GetAbil(AT_C_HITRATE))
	iAdd = kEffect:GetAbil(AT_R_MOVESPEED)
	InfoLog(9, "Effect_Begin1090004001 Before:AT_C_MOVESPEED:" .. unit:GetAbil(AT_C_MOVESPEED))
	kEffectMgr:AddAbil(AT_R_MOVESPEED, iAdd)
	unit:NftChangedAbil(AT_R_MOVESPEED, E_SENDTYPE_NONE)
	InfoLog(9, "Effect_Begin1090004001 After:AT_C_MOVESPEED:" .. unit:GetAbil(AT_C_MOVESPEED))
...
function Effect_End1090004001(unit, iEffectNo, actarg)
	--InfoLog(9, "Effect_End101000101 --")
	local kEffectMgr = unit:GetEffectMgr()
	local kEffect = kEffectMgr:GetEffect(iEffectNo, false)
	if kEffect:IsNil() == true then
		InfoLog(5, "Effect_End1090004001...Cannot GetEffect(" .. iEffectNo ..")");
		return 0
	end
	local iAdd = kEffect:GetAbil(AT_R_HITRATE)
...
function Effect_Tick1090004001(unit, elapsedtime, effect, actarg)
