...
-- Paladin (성스러운 보호 : a_Holy Armor) : Level 1-5
function Effect_Begin105501301(unit, iEffectNo, actarg)
	local effect = unit:GetEffect(iEffectNo);
	local kEffectTargetList = effect:GetTargetList();

	--InfoLog(5, "Effect_Begin105501301 : iEffectNo : "..iEffectNo)

--	kEffectTargetList:Add(unit:GetGuid());

	-- 실제 효과 이펙트	
	local iNewEffectNo = effect:GetAbil(AT_EFFECTNUM1);
	--InfoLog(5, "Effect_Begin105501301 : iNewEffectNo : "..iNewEffectNo)
--	unit:AddEffect(iNewEffectNo, 0, actarg, unit)
	...
	if kGround:IsNil() then
		InfoLog(5, "Effect_Tick105501301 Ground is NILL")
		return ECT_MUSTDELETE
	end
	...
	if unit:GetPartyGuid():IsNil() == false then
		...
		--InfoLog(5, "Effect_Tick105501301 : iRange "..iRange)
		while kMemberGuid:IsNil() == false do
			--InfoLog(5, "Effect_Tick105501301 : Party: index "..iIndex)
			-- 타겟 리스트에 없으면
			if false == kEffectTargetList:IsGuid(kMemberGuid) then
				--InfoLog(5, "Effect_Tick105501301 : kEffectTargetList:IsGuid() == false")
			...
			-- 타겟 리스트에 있으면
				...
				if kMember:IsNil() == false then
					...
					if IsInRange(kMyPos, kMember:GetPos(), iRange, 30) == false then
						--InfoLog(5, "Effect_Tick105501301 : InTargetList : Not InRange ")
						kEffectTargetList:Delete(kMemberGuid);
						kMember:DeleteEffect(iNewEffectNo);
						kMember:SetDeliverDamageUnit(GUID());
					-- Effect 사거리안에 있으면
					else
						--InfoLog(5, "Effect_Tick105501301 : InTargetList : InRange ")
...
function Effect_End105501301(unit, iEffectNo, actarg)
	local effect = unit:GetEffect(iEffectNo);
	local kGround = actarg:Get(ACTARG_GROUND)
	kGround = Ground(kGround);
...
function Effect_Tick105501301(unit, elapsedtime, effect, actarg)
	...
	--InfoLog(5, "Effect_Tick105501301 : iNewEffectNo : "..iNewEffectNo)
	--NeedMP값이 -로 세팅되어있다.
	if -iNeed > iMP then
		--InfoLog(5, "Effect_Tick105501301 : NeedMP : "..iNeed.. "  MP : "..iMP)
	...
	--InfoLog(9, "Effect_Tick105501301 NeedMP : " .. iNeed.."   C_MP:"..iMP)
...
-- Paladin (디펜시브 오라<효과> : a_Offecsive Aura) : Level 1-5
function Effect_Begin1055013001(unit, iEffectNo, actarg)
	local kEffectMgr = unit:GetEffectMgr()
	local kEffect = kEffectMgr:GetEffect(iEffectNo, false)
	if kEffect:IsNil() == true then
		InfoLog(5, "Effect_Begin1055013001...Cannot GetEffect(" .. iEffectNo ..")");
		return 0
	end
...
function Effect_End1055013001(unit, iEffectNo, actarg)
	local kEffectMgr = unit:GetEffectMgr()
	local kEffect = kEffectMgr:GetEffect(iEffectNo, false)
	if kEffect:IsNil() == true then
		InfoLog(5, "Effect_End1055013001...Cannot GetEffect(" .. iEffectNo ..")");
		return 0
	end
...
function Effect_Tick1055013001(unit, elapsedtime, effect, actarg)
	InfoLog(6, "Don't call me Effect_Tick1055013001 Effect=" .. effect:GetEffectNo())
	return ECT_DOTICK
end
