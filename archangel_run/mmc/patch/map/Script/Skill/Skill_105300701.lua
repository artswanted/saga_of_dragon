...
-- 기사 (어드벤스드 소드 마스터리 :a_Adv Sword Mastery) : Level 1-10 PASSIVE
function Skill_Passive105300701(unit, skillno, actarg)
	if unit:CheckPassive(skillno, actarg) == false then
		return -1
	end
	return Skill_PCheck105300701(unit, skillno, actarg)
end

function Skill_PCheck105300701(caster, skillno, actarg)
	local bOn = false
	local kSkillDef = GetSkillDef(skillno)
	if kSkillDef:IsNil() == true then
		InfoLog(5, "Skill_PCheck105300701 : Cannot Get SkillDef:" .. skillno)
		return -1
	end
	...
	if caster:CheckPassive(skillno, actarg) == false then
		if bOn == true then
			Skill_End105300701(caster, kSkillDef, actarg)
		end
		return -1
	end
	
	local kInven = caster:GetInventory()
	local kShield = kInven:GetItem(IT_FIT, EQUIP_POS_SHEILD)
	if kShield:IsNil() == true then
		if bOn==true then
			Skill_End105300701(caster, kSkillDef, actarg)
		end
	elseif bOn == false then
		Skill_Begin105300701(caster, kSkillDef, actarg)
	end

	return 0
end

function Skill_Begin105300701(caster, kSkillDef, actarg)
	InfoLog(9, "Skill_Begin105300701 --")
	local iEffectNo = kSkillDef:GetEffectNo()
...
function Skill_End105300701(caster, kSkillDef, actarg)
InfoLog(9, "Skill_End105300701 --")
	local iEffectNo = kSkillDef:GetEffectNo()
