...
function Skill_Passive110002301(unit, skillno, actarg)
	if unit:CheckPassive(skillno, actarg) == false then
		return -1
	end
	--Skill_CS_Passive110002301(unit, iSkillID)
	
	return Skill_PCheck110002301(unit, skillno, actarg)
end

function Skill_PCheck110002301(caster, skillno, actarg)
	local bOn = false
	local kSkillDef = GetSkillDef(skillno)
	if kSkillDef:IsNil() == true then
		InfoLog(5, "Skill_PCheck110002301 : Cannot Get SkillDef:" .. skillno)
		return -1
	end
	...
	if kEffect:IsNil() == false then
		InfoLog(9, "Skill_PCheck110002301 11")
		bOn = true	-- 이미 존재하는 Effect 이다.
	end
	if caster:CheckPassive(skillno, actarg) == false then
		if bOn == true then
			Skill_End110002301(caster, kSkillDef, actarg)
		end
		return -1
	end
	...
	if bOn == false	then
		Skill_Begin110002301(caster, kSkillDef, actarg)
	end
	return 0
end

function Skill_Begin110002301(caster, kSkillDef, actarg)
	--InfoLog(9, "Skill_Begin110002301 --")
	local iEffectNo = kSkillDef:GetEffectNo()
...
function Skill_End110002301(caster, kSkillDef, actarg)
	--InfoLog(9, "Skill_End110002301 --")
	local iEffectNo = kSkillDef:GetEffectNo()
