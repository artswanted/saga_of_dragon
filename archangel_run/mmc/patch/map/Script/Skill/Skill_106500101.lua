...
function Skill_Passive106500101(unit, skillno, actarg)
	if unit:CheckPassive(skillno, actarg) == false then
		return -1
	end
	--Skill_CS_Passive106500101(unit, iSkillID)
	
	return Skill_PCheck106500101(unit, skillno, actarg)
end

function Skill_PCheck106500101(caster, skillno, actarg)
	local bOn = false
	local kSkillDef = GetSkillDef(skillno)
	if kSkillDef:IsNil() == true then
		InfoLog(5, "Skill_PCheck106500101 : Cannot Get SkillDef:" .. skillno)
		return -1
	end
	...
	if kEffect:IsNil() == false then
		InfoLog(9, "Skill_PCheck106500101 11")
		bOn = true	-- 이미 존재하는 Effect 이다.
	end
	if caster:CheckPassive(skillno, actarg) == false then
		if bOn == true then
			Skill_End106500101(caster, kSkillDef, actarg)
		end
		return -1
	end
	...
	if bOn == false	then
		Skill_Begin106500101(caster, kSkillDef, actarg)
	end
	return 0
end

function Skill_Begin106500101(caster, kSkillDef, actarg)
	--InfoLog(9, "Skill_Begin106500101 --")
	local iEffectNo = kSkillDef:GetEffectNo()
...
function Skill_End106500101(caster, kSkillDef, actarg)
	--InfoLog(9, "Skill_End106500101 --")
	local iEffectNo = kSkillDef:GetEffectNo()
