...
function Skill_Passive104300101(unit, skillno, actarg)
	if unit:CheckPassive(skillno, actarg) == false then
		return -1
	end
	
	return Skill_PCheck104300101(unit, skillno, actarg)
end

function Skill_PCheck104300101(caster, skillno, actarg)
	local bOn = false
	local kSkillDef = GetSkillDef(skillno)
	if kSkillDef:IsNil() == true then
		InfoLog(5, "Skill_PCheck104300101 : Cannot Get SkillDef:" .. skillno)
		return -1
	end
	...
	if kEffect:IsNil() == false then
		InfoLog(9, "Skill_PCheck104300101 bOn true")
		bOn = true	-- 이미 존재하는 Effect 이다.
	end
	
	if caster:CheckPassive(skillno, actarg) == false then
		if bOn == true then
			Skill_End104300101(caster, kSkillDef, actarg)
		end
		return -1
	end
	
	-- 무기 착용 제한은 CheckPassive에서 이미 했다.
	Skill_Begin104300101(caster, kSkillDef, actarg)
	return 0
end

function Skill_Begin104300101(caster, kSkillDef, actarg)
	local iEffectNo = kSkillDef:GetEffectNo()
...
function Skill_End104300101(caster, kSkillDef, actarg)
	local iEffectNo = kSkillDef:GetEffectNo()
