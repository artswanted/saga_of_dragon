...
-- 매지션 (마법강화 : a_Spell Importance) : Level 1-10 PASSIVE
function Skill_Passive102002101(unit, skillno, actarg)
	if unit:CheckPassive(skillno, actarg) == false then
		return -1
	end
	
	return Skill_PCheck102002101(unit, skillno, actarg)
end

function Skill_PCheck102002101(caster, skillno, actarg)
	local bOn = false
	local kSkillDef = GetSkillDef(skillno)
	if kSkillDef:IsNil() == true then
		InfoLog(5, "Skill_PCheck102002101 : Cannot Get SkillDef:" .. skillno)
		return -1
	end
	...
	if caster:CheckPassive(skillno, actarg) == false then
		if bOn == true then
			Skill_End102002101(caster, kSkillDef, actarg)
		end
		return -1
	end
	...
	if bOn == false	then
		Skill_Begin102002101(caster, kSkillDef, actarg)
	end
	return 0
end

function Skill_Begin102002101(caster, kSkillDef, actarg)
	local iEffectNo = kSkillDef:GetEffectNo()
	
	local kEffect = caster:AddEffect(kSkillDef:GetEffectNo(), 0, actarg, caster)
	if kEffect:IsNil() == false then
		kEffect:SetActArgInt(ACTARG_WEAPONLIMIT, kSkillDef:GetAbil(AT_WEAPON_LIMIT))
	end
end

function Skill_End102002101(caster, kSkillDef, actarg)
	local iEffectNo = kSkillDef:GetEffectNo()
