...
function Skill_Passive106400001(unit, skillno, actarg)
	if unit:CheckPassive(skillno, actarg) == false then
		return -1
	end
	--Skill_CS_Passive105300101(unit, iSkillID)
	
	return Skill_PCheck106400001(unit, skillno, actarg)
end

function Skill_PCheck106400001(caster, skillno, actarg)
	local bOn = false
	local kSkillDef = GetSkillDef(skillno)
	if kSkillDef:IsNil() == true then
		InfoLog(5, "Skill_PCheck106400001 : Cannot Get SkillDef:" .. skillno)
		return -1
	end
	...
	if caster:CheckPassive(skillno, actarg) == false then
		if bOn == true then
			Skill_End106400001(caster, kSkillDef, actarg)
		end
		return -1
	end
	
	if bOn == false then
		Skill_Begin106400001(caster, kSkillDef, actarg)
	end

	return 0
end

function Skill_Begin106400001(caster, kSkillDef, actarg)
	local iEffectNo = kSkillDef:GetEffectNo()
...
function Skill_End106400001(caster, kSkillDef, actarg)
	local iEffectNo = kSkillDef:GetEffectNo()
