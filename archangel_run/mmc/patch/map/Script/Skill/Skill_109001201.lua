...
function Skill_Passive109001201(unit, skillno, actarg)
	if unit:CheckPassive(skillno, actarg) == false then
		return -1
	end
	
	return Skill_PCheck109001201(unit, skillno, actarg)
end

function Skill_PCheck109001201(caster, skillno, actarg)
	local bOn = false
	local kSkillDef = GetSkillDef(skillno)
	if kSkillDef:IsNil() == true then
		InfoLog(5, "Skill_PCheck109001201 : Cannot Get SkillDef:" .. skillno)
		return -1
	end
	...
	if caster:CheckPassive(skillno, actarg) == false then
		if bOn == true then
			Skill_End109001201(caster, kSkillDef, actarg)
		end
		return -1
	end
	...
	if bOn == false	then
		Skill_Begin109001201(caster, kSkillDef, actarg)
	end
	return 0
end

function Skill_Begin109001201(caster, kSkillDef, actarg)
	local iEffectNo = kSkillDef:GetEffectNo()
...
function Skill_End109001201(caster, kSkillDef, actarg)
	local iEffectNo = kSkillDef:GetEffectNo()
