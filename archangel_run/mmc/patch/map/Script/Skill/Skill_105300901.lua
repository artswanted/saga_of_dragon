...
function Skill_Passive105300901(unit, skillno, actarg)
	if unit:CheckPassive(skillno, actarg) == false then
		return -1
	end
	return Skill_PCheck105300901(unit, skillno, actarg)
end

function Skill_PCheck105300901(caster, skillno, actarg)
	local bOn = false
	local kSkillDef = GetSkillDef(skillno)
	if kSkillDef:IsNil() == true then
		InfoLog(5, "Skill_PCheck105300901 : Cannot Get SkillDef:" .. skillno)
		return -1
	end
	...
	if kEffect:IsNil() == false then
		...
		InfoLog(9, "Skill_PCheck105300901 bOn=true")
	end
	
	if caster:CheckPassive(skillno, actarg) == false then
		if bOn == true then
			Skill_End105300901(caster, kSkillDef, actarg)
		end
		return -1
	end
	
	if bOn == false then
		Skill_Begin105300901(caster, kSkillDef, actarg)
	end
	return 0
end

function Skill_Begin105300901(caster, kSkillDef, actarg)
	InfoLog(9, "Skill_Begin105300901 --")
	local iEffectNo = kSkillDef:GetEffectNo()
...
function Skill_End105300901(caster, kSkillDef, actarg)
	InfoLog(9, "Skill_End105300901 --")
	local iEffectNo = kSkillDef:GetEffectNo()
