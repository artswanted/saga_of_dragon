...
-- 길드 - 발빠른 대처(래피드 스텝)
function Skill_Passive200000001(unit, skillno, actarg)
	if unit:CheckPassive(skillno, actarg) == false then
		return -1
	end
	--Skill_CS_Passive101300101(unit, iSkillID)
	
	return Skill_PCheck200000001(unit, skillno, actarg)
end

function Skill_PCheck200000001(caster, skillno, actarg)
	local bOn = false
	local kSkillDef = GetSkillDef(skillno)
	if kSkillDef:IsNil() == true then
		InfoLog(5, "Skill_PCheck200000001 : Cannot Get SkillDef:" .. skillno)
		return -1
	end
	...
	if kEffect:IsNil() == false then
		InfoLog(9, "Skill_PCheck200000001 11")
		bOn = true	-- 이미 존재하는 Effect 이다.
	end
	if caster:CheckPassive(skillno, actarg) == false then
		if bOn == true then
			Skill_End200000001(caster, kSkillDef, actarg)
		end
		return -1
	end
	...
	if bOn == false	then
		Skill_Begin200000001(caster, kSkillDef, actarg)
	end
	return 0
end

function Skill_Begin200000001(caster, kSkillDef, actarg)
	InfoLog(9, "Skill_Begin200000001 --")
	local iEffectNo = kSkillDef:GetEffectNo()
...
function Skill_End200000001(caster, kSkillDef, actarg)
	InfoLog(9, "Skill_End200000001 --")
	local iEffectNo = kSkillDef:GetEffectNo()
