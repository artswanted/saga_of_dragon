...
function Skill_Passive160000001(unit, skillno, actarg)
	if unit:CheckPassive(skillno, actarg) == false then
		return -1
	end
	
	return Skill_PCheck160000001(unit, skillno, actarg)
end

function Skill_PCheck160000001(caster, skillno, actarg)
	local bOn = false
	local kSkillDef = GetSkillDef(skillno)
	if kSkillDef:IsNil() == true then
		InfoLog(5, "Skill_PCheck160000001 : Cannot Get SkillDef:" .. skillno)
		return -1
	end
	...
	if caster:CheckPassive(skillno, actarg) == false then
		if bOn == true then
			Skill_End160000001(caster, kSkillDef, actarg)
		end
		return -1
	end
	...
	if bOn == false	then
		Skill_Begin160000001(caster, kSkillDef, actarg)
	end
	return 0
end

function Skill_Begin160000001(caster, kSkillDef, actarg)
	local iEffectNo = kSkillDef:GetEffectNo()
...
function Skill_End160000001(caster, kSkillDef, actarg)
	local iEffectNo = kSkillDef:GetEffectNo()
