...
function Skill_Begin143(caster, skillnum, iStatus,arg, TailPacket)
	return Skill_BeginDefault(caster, skillnum, iStatus,arg)
end

function Skill_Fire143(caster, kTargetArray, skillnum, result, actarg)
	-- GetSkillResult143(caster, kTargetArray, skillnum, result)
	local kSkillDef = GetSkillDef(skillnum)
	if kSkillDef:IsNil() then
		InfoLog(5, "Skill_Fire143....SkillDef is NIl : " ..skillnum)
		return -1
	end

	local target = kTargetArray:GetUnit(0)
	if target:IsNil() then
		InfoLog(5, "Skill_Fire143....Target is NIl : " ..skillnum)
		return -1
	end
	...
	if slave:IsNil() then	--주변에 잡몹이 없음
		InfoLog(5, "Skill_Fire143....Slave is NIl : " ..skillnum)
		return -1
	end
	...
	while slave:IsNil() == false do
		local targetGuid = slave:GetTarget()
		if targetGuid:IsNil() and false == slave:GetGuid():IsEqual(caster:GetGuid()) then
...
function Skill_Fail143(caster, kTargetArray, skillnum, result, arg)
	local iIndex = 0
...
function Skill_CanReserve143(caster, kTargetArray, skillnum, actarg)
