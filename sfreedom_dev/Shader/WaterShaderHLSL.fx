...
float4x4 TextureTransform : TexTransformBase;
...
VS_OUTPUT_NO_PS VS_WaterShader_NO_PS(float4 Position : POSITION,float4 VertexColor : COLOR0,float2 TexCoord0 : TEXCOORD0)
{
    VS_OUTPUT_NO_PS OUT = (VS_OUTPUT_NO_PS)0;
    OUT.Position = mul(Position, WorldViewProj);
    OUT.TexCoord0 = mul(float4(TexCoord0.x, TexCoord0.y, 0.0, 1.0), TextureTransform);
    OUT.Diffuse = VertexColor;
	return	OUT;
}

VS_OUTPUT VS_WaterShader(float4 Position : POSITION,float2 TexCoord0 : TEXCOORD0)
{
	...
	OUT.TexCoord7 = mul(float4(TexCoord0.x, TexCoord0.y, 0.0, 1.0), TextureTransform);
...
{   
	...
	float4 r3 = tex2D(sWaterDetail,In.TexCoord7);//r0);
...
>
{
	pass One 
	{
		...
		ZEnable = true;
		ZWriteEnable = true;
		VertexShader = compile vs_1_1 VS_WaterShader_NO_PS();
		PixelShader  = NULL;
		
        Texture[0] = (WaterDetail);
        ColorOp[0] = Modulate;
        ColorArg1[0] = Texture;
        ColorArg2[0] = Diffuse;
