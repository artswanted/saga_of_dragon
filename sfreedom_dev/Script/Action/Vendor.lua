function Act_vendor_OnCheckCanEnter(actor, action)
...
function Act_vendor_OnEnter(actor, action)
	if( CheckNil(actor == nil) ) then return false end
	if( CheckNil(actor:IsNil()) ) then return false end	
	...
	ODS("Act_vendor_OnEnter",false, 912)
	...
	if ptTargetPos ~= nil or ptTargetPos:IsNil() == false then
		...
		actor:AttachParticleToPointWithRotate(99100, TargetPos, "ef_vendor_char_root", actor:GetRotateQuaternion(), 1.0)
...
function Act_vendor_OnUpdate(actor, accumTime, frameTime)
	if( CheckNil(actor == nil) ) then return false end
	if( CheckNil(actor:IsNil()) ) then return false end	
...
function Act_vendor_OnLeave(actor, action)	
	...
	local iCurrentSlot = kCurAction:GetCurrentSlot()
	local bIsAnimDone = actor:IsAnimationDone()
...
function Act_vendor_OnCleanUp(actor, action)
	if( CheckNil(actor == nil) ) then return true end
	if( CheckNil(actor:IsNil()) ) then return true end	
...
function Act_vendor_OnClearUpRun(actor)
	if( CheckNil(actor == nil) ) then return false end
	if( CheckNil(actor:IsNil()) ) then return false end		
