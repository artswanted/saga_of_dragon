--[[ Teleport 
100번 slot에 eventType이 들어온다.
	0 : 땅으로 들어가고, 점프해서 나오기 (Map_Teleport 에서 불림)
	1 : 걸어 들어가고 걸어 나오기 (Map_EnterDugeon)
	2 : 바로 이동해서 점프해서 나오기 (Touch_Teleport)
	3 : 파티원 전체를 목표지점으로 강제 이동(Party_Map_EnterDugeon) (시작)
	4 : 파티원 전체를 목표지점으로 강제 이동 후, 파티원들이 이동 할 때 (후: 시작한 본인 뺴고 전부)
	5 : 땅으로 들어가고, 점프해서 나오기 페이트in/out 연출 없음 (Telewarp 에서 불림)
	8 : 파티원들 강제로 9번 이벤트 타입 발동
	9 : 2번타입과 동일하지만 데미지 없는 위치이동
]]

function Act_Teleport_OnEnter(actor, action)
	...
	--g_TeleDestPos = action:GetParamAsPoint(1)
	...
	if packet ~= nil and packet:IsNil() == false then
		...
		--ODS("Act_Teleport_OnEnter actor:"..actor:GetPilot():GetName():GetStr().." kDestTrn : "..ptTargetPos:GetX()..","..ptTargetPos:GetY()..","..ptTargetPos:GetZ().."\n");
	end
	...
	if eventType == 1 or eventType == 3 or eventType == 4 or 6 == eventType or 7 == eventType or eventType == 8 then 
		if action:GetParam(1423) ~= "ladder" then
			...
			elseif 6 == eventType or 7 == eventType then			
				...
				if actor:IsRidingPet() then
					...
					action:SetSlot(3) --펫 탑승 중이라면 일어나지 말자
					--나는 앉아있고 펫 액션은 걷기로 전환
					kActionPet:SetSlot(4)
					kActorPet:PlayCurrentSlot()
				else
					action:SetSlot(1) --걸어들어감
				end				
			end
		else
			action:SetSlot(2)
		...
		if( 6 == eventType ) then
			Net_PT_C_M_REQ_START_HYPER_MOVE(action:GetParam(914), 3)
		else
...
function Act_Teleport_OnOverridePacket(actor,action,packet)
...
function Act_Teleport_OnUpdate(actor, accumTime, frameTime)
		...
		if action:GetParamFloat(3) == 0.0 then
			...
			if action:GetParam(1423) ~= "ladder" then
				...
				if eventType ~= 4 then
					...
					elseif kDir == DIR_LEFT_UP or kDir == DIR_RIGHT_UP or kDir == DIR_LEFT_DOWN or kDir == DIR_RIGHT_DOWN then
						actor:SetLookingDirection(kDir, true)
					end
				else
					actor:SeeFront(true, true)
				end
	...
	elseif(6 == eventType or 7 == eventType) then
		...
		actor:Stop()
		action:SetParam(0,"end")
		if eventType ~= 5 or eventType ~= 9 then
			if actor:IsMyActor() then	-- damage 요청, trap코드랑 공유하고 있다.				
...
function Act_Teleport_OnCleanUp(actor, action)
	...
	if action:GetID() ~= "a_teleport_out" then
...
function Act_Teleport_OnLeave(actor, action)
...
--	if action:GetID() ~= "teleport_out" then
--		return false 
--	end
	local curAction = actor:GetAction()
...
-- Teleport Out
function Act_Teleport_Out_OnEnter(actor, action)
	CheckNil(actor == nil);
	CheckNil(actor:IsNil());
	...
	if packet ~= nil and packet:IsNil() == false then
		...
		ODS("Act_Teleport_Out_OnEnter actor:"..actor:GetPilot():GetName():GetStr().." kDestTrn : "..ptTargetPos:GetX()..","..ptTargetPos:GetY()..","..ptTargetPos:GetZ().."\n");
	...
	ODS("Act_Teleport_Out_OnEnter[1] actor:"..actor:GetPilot():GetName():GetStr().." kDestTrn : "..kDestTrn:GetX()..","..kDestTrn:GetY()..","..kDestTrn:GetZ().."\n");
	...
	if kDestTrn:IsZero() == false then
		...
		ODS("Act_Teleport_Out_OnEnter[1] actor to floor: "..kFloorPos:GetX()..","..kFloorPos:GetY()..","..kFloorPos:GetZ().."\n");
		...
		if eventType == 2 or eventType == 8 or eventType == 9 then
			kFloorPos:SetZ(kFloorPos:GetZ() + 75)
			actor:SetTranslate(kFloorPos)
			action:SetSlot(2)
			actor:StartJump(0)
		end
...
function Act_Teleport_Out_OnOverridePacket(actor,action,packet)
...
function Act_Teleport_Out_OnUpdate(actor, accumTime, frameTime)
...
function Act_Teleport_Out_OnCleanUp(actor, action)
...
function Act_Teleport_Out_OnLeave(actor, action)
	if action:GetID() == "a_teleport" then
		ODS("Act_Teleport_Out_OnLeave, a_teleport entered! \n");
		return false 
	end
	return true
end