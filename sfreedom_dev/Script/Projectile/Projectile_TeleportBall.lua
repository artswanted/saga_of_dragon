...
function PROJECTILE_TeleportBall_OnLoadingStart(kProjectile)	
	return	true;
end
function PROJECTILE_TeleportBall_OnLoading(kProjectile)
	return	true;
end

function PROJECTILE_TeleportBall_OnFlyingStart(kProjectile)
...
function PROJECTILE_TeleportBall_OnFlying(kProjectile)

	return	true;
end

function PROJECTILE_TeleportBall_OnCollision(kProjectile, kCollideActor, kTargets)
	--	Find Target
...
function PROJECTILE_TeleportBall_OnTargetListModified(kProjectile)

	SkillFunc_ProjectileHitOneTime(kProjectile)
	return	false;
end

function PROJECTILE_TeleportBall_OnUpdate(kProjectile)
	...
	if (iCurrentTime - iPrevTime) > 90 then
		...
		local iRet = PROJECTILE_TeleportBall_HitOneTime(kProjectile,kParentPilot:GetActor());
...
function PROJECTILE_TeleportBall_OnArrivedAtTarget(kProjectile)
	--ODS( GetAccumTime() ..  " PROJECTILE_TeleportBall_OnArrivedAtTarget >>>>>> \n", false, 998 )
	PROJECTILE_TeleportBall_OnTargetListModified(kProjectile)
...
function PROJECTILE_TeleportBall_OnAnimationEvent(kProjectile)
	return	true
end
