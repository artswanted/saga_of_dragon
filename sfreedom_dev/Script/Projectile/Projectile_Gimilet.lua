...
function PROJECTILE_GIMILET_OnFlyingStart(kProjectile)
	...
	g_world:AttachParticle("ef_Gimilet_01", ptcl);
	
	return	true;
end


function PROJECTILE_GIMILET_OnCollision(kProjectile,kCollideActor,kTargets)
...
function PROJECTILE_GIMILET_OnTargetListModified(kProjectile)
	return SkillFunc_ProjectileHitOneTime(kProjectile)
end


function PROJECTILE_GIMILET_OnArrivedAtTarget(kProjectile)
	ODS("PROJECTILE_GIMILET_OnArrivedAtTarget\n");

	PROJECTILE_GIMILET_OnTargetListModified(kProjectile);	

	return	false;
end
function PROJECTILE_GIMILET_OnAnimationEvent(kProjectile)
	return	true
end

function PROJECTILE_GIMILET_OnUpdate(kProjectile)
	...
	if fBoomElapsedTime >= 0.04 and iCount < 20 then
		...
		g_world:AttachParticleSWithRotate("ef_Gimilet_01", ptcl, 1.0, Point3(0,0,math.random(0, 360)));		
	end
